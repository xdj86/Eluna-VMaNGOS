...
{
    sLog.Out(LOG_BASIC, LOG_LVL_BASIC, "Loading alternative spec");

    // 清空内存中的旧数据
    m_altspec_talents.clear();

    // 使用 auto 自动推导类型，获得 unique_ptr
    auto result = CharacterDatabase.PQuery("SELECT altspells FROM character_altspec WHERE guid = '%u'", GetGUIDLow());

    if (!result) // unique_ptr 可以隐式转换为 bool
    {
        sLog.Out(LOG_BASIC, LOG_LVL_BASIC, "No saved alternative spec for player %u", GetGUIDLow());
        return;
    }

    // 使用 result-> 访问成员（unique_ptr 重载了 -> 操作符）
    Field* fields = result->Fetch();
    std::string altSpellsStr = fields[0].GetString();

    if (!altSpellsStr.empty())
    {
        std::istringstream iss(altSpellsStr);
        uint32 spellId;
        while (iss >> spellId)
        {
            m_altspec_talents.push_back(spellId);
        }
        sLog.Out(LOG_BASIC, LOG_LVL_BASIC, "Loaded %zu spells for alternative spec", m_altspec_talents.size());
    }
    else
    {
        sLog.Out(LOG_BASIC, LOG_LVL_BASIC, "Alternative spec string is empty for player %u", GetGUIDLow());
    }

    // 注意：不需要 delete result，unique_ptr 会自动释放内存
    sLog.Out(LOG_BASIC, LOG_LVL_BASIC, "Alternative spec loaded");
}

void Player::_SaveAlternativeSpec()
{
    
    sLog.Out(LOG_BASIC, LOG_LVL_BASIC, "Saving spec");
    ...
    for (SpellIDList::iterator it = m_altspec_talents.begin(); it != m_altspec_talents.end(); it++)
        ss << *it << " ";
    ss << "')";
    sLog.Out(LOG_BASIC, LOG_LVL_BASIC, "Spells serialized");

    // Nice, it saved!
    CharacterDatabase.PExecute(ss.str().c_str());
    sLog.Out(LOG_BASIC, LOG_LVL_BASIC, "Spells saved");
...
uint32 Player::SwapSpec()
{
    ...
    */
    ...
    /***                   SAVE TALENTS                    ***/
    /*********************************************************/
    sLog.Out(LOG_BASIC, LOG_LVL_BASIC, "Save talents");
    ...
    for (unsigned int i = 0; i < sTalentStore.GetNumRows(); ++i)
    {
        ...
        for (int j = 0; j < 5; ++j)
        {
            for (PlayerSpellMap::iterator itr = GetSpellMap().begin(); itr != GetSpellMap().end();)
            {
                ...
                if (itrFirstId == talentInfo->RankID[j] || sSpellMgr.IsSpellLearnToSpell(talentInfo->RankID[j], itrFirstId)) //|| HasSpell(spellmgr.GetSpellRequired(itrFirstId)))
    ...
    /***                   LOAD TALENTS                    ***/
    /*********************************************************/
    sLog.Out(LOG_BASIC, LOG_LVL_BASIC, " Load talents ");
